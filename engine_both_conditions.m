...
%engineStruct = analyzeEngine([30 1.2 2.0 0.018 0.010 0.1], givens, conditions);
...
f_ab_guess = 0.01;
b_guess = 0.1;
x0 = [Pr_comp_guess, Pr_fan_guess, beta_guess, f_guess, f_guess, f_ab_guess, f_ab_guess, b_guess, b_guess];

%define upper and lower bounds for design variables
lb = [0 requirements.Pr_fan_min 0 0 0 0 0 0 0];
ub = [requirements.Pr_comp_max requirements.Pr_fan_max requirements.beta_max inf inf inf inf requirements.b_max requirements.b_max];

%=====DOUBLE CONDITION ENGINE=====

%define flight condition inputs for optimization
conditions.Ta_cruise = 220; %K
conditions.Pa_cruise = 29e3; %Pa
conditions.M_cruise = 0.86;
requirements.ST_cruise = 0.86e3; %N*s/kg
conditions.Ta_takeoff = 285; %K
conditions.Pa_takeoff = 100e3; %Pa
conditions.M_takeoff = 0.0;
requirements.ST_takeoff = 2.8e3; %N*s/kg
...
"MaxIterations",1e6);
objectiveFunction = @(x) calculateST(x, givens, conditions);
nonlcon = @(x) constraints(x, givens, requirements, conditions);
[out, tsfc, ~, ~, ~, ~, ~] = fmincon(objectiveFunction, x0, [], [], [], [], lb, ub, nonlcon, options);

%outputs
[engineAtCruise, engineAtTakeoff] = analyzeEngine(out, givens, conditions);

%=====EXPORT ENGINE AT CRUISE=====

%calculate max fuel-air ratios
fmax = sym('fmax');
To4_max_adjusted = requirements.To4_max + givens.Cb1*sqrt(engineAtCruise.b/requirements.b_max);
Cp_comb = givens.R*(givens.gamma_comb/(givens.gamma_comb-1));
To4_expression = ((1-engineAtCruise.b)*engineAtCruise.To3 + (fmax*givens.eta_comb*givens.Q_fuel/Cp_comb))/(1+fmax-engineAtCruise.b);
implicitSol = To4_expression == To4_max_adjusted;
engineAtCruise.f_max = double(vpasolve(implicitSol, fmax, [0 inf]));

Cp_ab = givens.R*(givens.gamma_ab/(givens.gamma_ab-1));
To6_expression = ((1+engineAtCruise.f_max)*engineAtCruise.To52 + (fmax*givens.eta_ab*givens.Q_fuel/Cp_ab))/(1+engineAtCruise.f_max+fmax);
implicitSol = To6_expression == requirements.To6_max;
engineAtCruise.f_ab_max = double(vpasolve(implicitSol, fmax, [0 inf]));

%save final optimized result
disp('Double-Optimized Engine At Cruise:');
disp(engineAtCruise);
writetable(struct2table(engineAtCruise), 'data outputs/double_optimized_engine_at_cruise.xlsx');

%=====EXPORT ENGINE AT TAKEOFF=====

%calculate max fuel-air ratios
fmax = sym('fmax');
To4_max_adjusted = requirements.To4_max + givens.Cb1*sqrt(engineAtTakeoff.b/requirements.b_max);
Cp_comb = givens.R*(givens.gamma_comb/(givens.gamma_comb-1));
To4_expression = ((1-engineAtTakeoff.b)*engineAtTakeoff.To3 + (fmax*givens.eta_comb*givens.Q_fuel/Cp_comb))/(1+fmax-engineAtTakeoff.b);
implicitSol = To4_expression == To4_max_adjusted;
engineAtTakeoff.f_max = double(vpasolve(implicitSol, fmax, [0 inf]));

Cp_ab = givens.R*(givens.gamma_ab/(givens.gamma_ab-1));
To6_expression = ((1+engineAtTakeoff.f_max)*engineAtTakeoff.To52 + (fmax*givens.eta_ab*givens.Q_fuel/Cp_ab))/(1+engineAtTakeoff.f_max+fmax);
implicitSol = To6_expression == requirements.To6_max;
engineAtTakeoff.f_ab_max = double(vpasolve(implicitSol, fmax, [0 inf]));

%save final optimized result
disp('Double-Optimized Engine At Takeoff:');
...
function ST = calculateST(x, givens, conditions)
    %perform analysis
    [engineAtCruise, engineAtTakeoff] = analyzeEngine(x, givens, conditions);

    %st objective
    ST = -sum([engineAtCruise.ST engineAtTakeoff.ST]);
end

%nonlinear constraint function
function [c, ceq] = constraints(x, givens, requirements, conditions)
    %perform analysis
    [engineAtCruise, engineAtTakeoff] = analyzeEngine(x, givens, conditions);

    %=====ENGINE AT CRUISE=====
    %calculate specific thrust and constrain
    c(1) = requirements.ST_cruise - engineAtCruise.ST;
    %calculate To4 and constrain
    To4_max_adjusted = requirements.To4_max + givens.Cb1*sqrt(engineAtCruise.b/requirements.b_max);
    c(end+1) = engineAtCruise.To4 - To4_max_adjusted;
    %constrain To6
    c(end+1) = engineAtCruise.To6 - requirements.To6_max;

    %=====ENGINE AT TAKEOFF=====
    %calculate specific thrust and constrain
    c(end+1) = requirements.ST_takeoff - engineAtTakeoff.ST;
    %calculate To4 and constrain
    To4_max_adjusted = requirements.To4_max + givens.Cb1*sqrt(engineAtTakeoff.b/requirements.b_max);
    c(end+1) = engineAtTakeoff.To4 - To4_max_adjusted;
    %constrain To6
    c(end+1) = engineAtTakeoff.To6 - requirements.To6_max;

    ceq = [];
end

%unpacking function
function [Pr_comp, Pr_fan, beta, f_cruise, f_takeoff, f_ab_cruise, f_ab_takeoff, b_cruise, b_takeoff] = unpack(x)
    Pr_comp = x(1);
    Pr_fan = x(2);
    beta = x(3);
    f_cruise = x(4);
...
function [engineAtCruise, engineAtTakeoff] = analyzeEngine(x, givens, conditions)
    %=====COMPONENT FRONT-TO-BACK ANALYSIS AT CRUISE=====
    %define flight conditions
    givens.Ta = conditions.Ta_cruise;
    givens.Pa = conditions.Pa_cruise;
    givens.M = conditions.M_cruise;
    ...
    %=====EXPORT ENGINE AT CRUISE=====
    engineAtCruise = engineStruct;
    
    %=====COMPONENT FRONT-TO-BACK ANALYSIS AT TAKEOFF=====
    %define flight conditions
    givens.Ta = conditions.Ta_takeoff;
    givens.Pa = conditions.Pa_takeoff;
    givens.M = conditions.M_takeoff;
    %unpack design variables
    [engineStruct.Pr_comp, engineStruct.Pr_fan, engineStruct.beta, ~, engineStruct.f, ~, engineStruct.f_ab, ~, engineStruct.b] = unpack(x);
    %diffuser exit properties
    [engineStruct.Po2, engineStruct.To2] = diffuser(givens);
